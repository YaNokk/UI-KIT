...
import { countSeparatorsFromHead, getValueSegments, parseDateRangeString, removeSeparatorsFromTail, replaceSegmentsData, segmentsToString, shiftSegmentsData, validateSegments, } from './utils';
export function createDisallowInputPreprocessor() {
    return (state, actionType) => {
        const { elementState, data } = state;
        const { value, selection: [from, to], } = elementState;
...
export function createValidationPreprocessor(template, fullStringTemplate, min, max, onCorrection) {
    ...
    return (state, actionType) => {
        ...
        if (!rawValue || (isValidation && from !== to))
            return state;
        ...
        if (!isValidation) {
            ...
            if (clearPossibleValue !== clearValidatedValue)
                onCorrection();
        }
...
function insertNewData({ elementState, validatedValue, rawValue, possibleValue, toSelection, }) {
    const { selection: [from, to], } = elementState;
    ...
    const newData = validatedValue.slice(from + existSegmentSeparator, toSelection - selectedChars + addedDateSegmentSeparators + existSegmentSeparator);
    const newValue = validatedValue.slice(0, from + existSegmentSeparator) +
...
function getMinMax({ hasRangeSeparator, dateIdx, dateStrings, dateTemplate, }) {
