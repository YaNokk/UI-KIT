 ...
 */
export function getValueSegments(rawValue, separators) {
 ...
 */
function prepareValueForSegmentation(value) {
    if (value.indexOf(' ') !== -1) {
        if (value.indexOf(DATE_RANGE_SEPARATOR) === -1 &&
        ...
        if (value.indexOf(DATE_TIME_SEPARATOR) === -1 &&
 ...
 */
export function shiftSegmentsData(rawSegments, templateSegments) {
 ...
 */
export function replaceSegmentsData(rawSegments, template, selection, data) {
    const segments = [...rawSegments];
    ...
    template.segments.forEach((templateSegment, idx, arr) => {
        ...
        if (segment && templateSegment.length < segment.length) {
            ...
            if (segmentIdx === idx && offset !== undefined) {
                ...
                segments[idx] = (segment.slice(0, offset) +
                    data +
                    segment.slice(offset + overflowLen + data.length)).slice(0, templateSegment.length);
            }
            else {
 ...
 */
export function findCursorPlace(segments, templateSegments, separators, selection) {
    let cursorPlace;
    let segmentIdx = 0;
    let posStart = 0;
 ...
 */
export function segmentsToPattern(segments, separators) {
 ...
 */
export function segmentsToString(segments, separators) {
    const str = segments
        .map((p, idx) => {
 ...
 *  01.12. -> 01.12
 */
export function removeSeparatorsFromTail(str) {
 ...
 */
export function countSeparatorsFromHead(str, startFrom = 0) {
 ...
 *  9.9 -> 09.09
 */
export function validateSegments({ dateTemplate, templateSegments, segments, selection: [from, to], min, max, }) {
...
export function minMaxValidation(segments, min, max) {
    ...
    if (dayFilled && monthFilled && yearFilled) {
        const date = clamp(new Date(`${segments.year}-${segments.month}-${segments.day}T00:00:00`), {
            start: min,
            end: max,
        });
...
export function padWithZeroesUntilValid(segmentValue, paddedMaxValue, prefixedZeroesCount = 0) {
 ...
 */
export function toDateString({ day, month, year, hours, minutes }, dateTemplate) {
    return dateTemplate
        .replace(/d+/g, day ?? '')
        .replace(/MM+/g, month ?? '')
        .replace(/y+/g, year ?? '')
...
export function parseDateRangeString(dateRange, maxLen) {
    return dateRange.split(DATE_RANGE_SEPARATOR).map((date) => date.slice(0, maxLen));
}
export function segmentsToObj(segments, templateSegments) {
    return {
