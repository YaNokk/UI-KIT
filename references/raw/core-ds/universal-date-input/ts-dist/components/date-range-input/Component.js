...
import { formatDate, formatDateRange, getValidRange, isCompleteDate, isCompleteDateRange, parseDateString, preventDefault, updateRange, } from '../../utils';
...
export const DateRangeInput = forwardRef(({ autoCorrection, breakpoint, wrapperClassName, open, onCalendarClose, value: valueProp, defaultValue, inputWrapperRef: inputWrapperRefProp = null, onInputChange, onChange, onBlur, Calendar, wrapperHandlers, calendarRef, calendarProps = {}, minDate = DEFAULT_MAX_DATE, maxDate = DEFAULT_MIN_DATE, block, dataTestId, error, popoverProps, rightAddons, picker, platform, onPickerClick, rangeBehavior = 'clarification', clear, onClear, ...restProps }, ref) => {
    ...
    const { validFrom, validTo } = useMemo(() => getValidRange({ from, to, offDays, minDate, maxDate }), [from, to, offDays, maxDate, minDate]);
    useEffect(() => {
    ...
    useEffect(() => {
        if (dateFromProp !== undefined && dateToProp !== undefined) {
            setInputValue(dateFromProp && dateToProp && isValid(dateFromProp) && isValid(dateToProp)
    ...
    const changeInputValue = (val, event) => {
        ...
        setInputValue(val);
        if (val === '' || isCompleteDateRange(val))
            callOnChange(val);
    };
    ...
    const renderCalendar = () => {
        if (picker && Calendar) {
            ...
            React.createElement("div", { onMouseDown: preventDefault, className: styles.calendarWrapper },
                React.createElement(Calendar, { dataTestId: getDataTestId(dataTestId, 'calendar'), month: calendarMonth, ...calendarProps, onMonthChange: handleMonthChange, breakpoint: breakpoint, responsive: true, open: open, onClose: onCalendarClose, ref: calendarRef, selectedFrom: validFrom ? validFrom.getTime() : undefined, selectedTo: validTo ? validTo.getTime() : undefined, value: undefined, onChange: handleCalendarChange, minDate: minDate, maxDate: maxDate })));
        }
        return null;
    };
    ...
    return (React.createElement("div", { ...wrapperHandlers, className: cn(styles.wrapper, wrapperClassName, {
        ...
        React.createElement(Input, { ...restProps, clear: clear && isCompleteDateRange(inputValue), onClear: valueProp === undefined ? handleClear : onClear, dataTestId: dataTestId, breakpoint: breakpoint, wrapperRef: mergeRefs([inputWrapperRef, inputWrapperRefProp]), ref: mergeRefs([ref, inputRef]), value: inputValue, inputMode: 'decimal', onInput: handleInputChange, onBlur: handleBlur, error: error || getInnerError(), block: true, rightAddons: React.createElement(React.Fragment, null,
                rightAddons,
