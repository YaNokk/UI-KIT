...
export function parseDateString(value, format = DATE_FORMAT) {
    return parse(value, format, new Date());
}
export const formatDate = (date, dateFormat = DATE_FORMAT) => dateFnsFormat(date, dateFormat);
...
export function isValidDate({ value = '', minDate, maxDate, offDays, }) {
    ...
    if (parsed) {
        return (parsed >= minDateStartOfDay &&
            parsed <= maxDate &&
...
export function getValidRange({ from, to, offDays, minDate, maxDate, }) {
 ...
 */
export function updateRange({ date, validFrom, validTo, rangeBehavior, }) {
    ...
    if (rangeBehavior === 'clarification') {
        ...
        else if (Math.abs(differenceInDays(date, selectedTo)) >
...
export function getUniversalDateInputTestIds(dataTestId) {
    return {
        input: dataTestId,
