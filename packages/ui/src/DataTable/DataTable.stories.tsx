...
function expectSelectionControlsCentered(canvasElement: HTMLElement) {
  const selectionCells = canvasElement.querySelectorAll<HTMLElement>(".ds-table-selection-cell");
  expect(selectionCells.length).toBeGreaterThan(1);
  for (const cell of selectionCells) {
    const control = cell.firstElementChild;
    if (!(control instanceof HTMLElement)) throw new Error("Selection control is unavailable");
    ...
    expect(Math.abs(controlCenter - cellCenter)).toBeLessThanOrEqual(0.1);
  }
}
...
export const SelectionExplicitGeometryRegression: Story = { tags: ["test-only"], render: () => <Demo selectionMode="explicit" />, play: async ({ canvasElement }) => { const canvas = within(canvasElement); const initial = captureTableGeometry(canvasElement); expectSelectionControlsCentered(canvasElement); const firstRow = canvas.getByRole("checkbox", { name: "Выбрать строку 1042" }); await userEvent.click(firstRow); await expect(canvas.getByRole("checkbox", { name: "Выбрать строки на странице" })).toBePartiallyChecked(); expectStableTableGeometry(initial, captureTableGeometry(canvasElement)); expectSelectionControlsCentered(canvasElement); await userEvent.click(canvas.getByRole("checkbox", { name: "Выбрать строки на странице" })); await expect(canvas.getByRole("checkbox", { name: "Выбрать строки на странице" })).toBeChecked(); expectStableTableGeometry(initial, captureTableGeometry(canvasElement)); expectSelectionControlsCentered(canvasElement); } };
